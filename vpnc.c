...
struct isakmp_attribute *make_transform_ike(int dh_group, int crypt, int hash, int keylen)
{
	...
	*((uint32_t *) a->u.lots.data) = htonl(2147483);
	...
	a = new_isakmp_attribute_16(IKE_ATTRIB_AUTH_METHOD, XAUTH_AUTH_XAUTHInitPreShared, a);
...
void do_phase_1(const char *key_id, const char *shared_key, struct sa_block *d)
{
	...
	/* Decode the recieved packet.  */
	{
		...
		if (reject == 0 && memcmp(r->i_cookie, d->i_cookie, ISAKMP_COOKIE_LENGTH) != 0)
		...
		if (reject == 0 && (hash == NULL || hash->u.hash.length != d->md_len))
			reject = ISAKMP_N_INVALID_HASH_INFORMATION;
		if (reject != 0)
			...
			if (d->keylen > d->md_len) {
				for (i = 0; i * d->md_len < d->keylen; i++) {
					...
					if (i == 0)
						...
						gcry_md_write(hm, d->key + (i - 1) * d->md_len,
							d->md_len);
					gcry_md_final(hm);
...
static int do_phase_2_xauth(struct sa_block *s)
{
	...
	for (loopcount = 0;; loopcount++) {
		...
		if (reject == 0 &&
			...
			r->payload->next != NULL && r->payload->next->type == ISAKMP_PAYLOAD_N) {
...
void read_config_file(char *name, const char **configs, int missingok)
{
	FILE *f;
	char *line = NULL;
...
void print_desc(const char *pre, const char *text)
{
	const char *p, *q;
	...
	for (i = 1; i < argc; i++)
		if (argv[i][0] == '-') {
			...
			for (c = 0; config_names[c].name != NULL && !known; c++)
				if (config_names[c].option != NULL
					...
					if (argv[i][strlen(config_names[c].option)] == '=')
						s = strdup(argv[i] +
							strlen(config_names[c].option) + 1);
			...
			if (!known && strcmp(argv[i], "--version") == 0) {
				...
				for (i = 0; i < sizeof(supp_dh_group) / sizeof(supp_dh_group[0]);
					i++)
	...
	if (print_config) {
		...
		for (i = 0; config_names[i].name != NULL; i++)
			if (config[config_names[i].nm] != NULL)
				printf("%s%s\n", config_names[i].name,
					(config_names[i].needsArgument) ? config[config_names[i].
						nm] : "");
		exit(0);
	}
